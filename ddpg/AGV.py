...
class AGV:
    ...
    def countHB(self):
        return np.dot(np.dot(self.getSumB().T,self.M),np.dot(np.dot(np.dot(self.getEB().T,self.If),self.getEB()),self.getSumB()))
...
#
# a.uk = np.matrix([[10], [np.pi/10]])
a.uk = np.matrix([[0, 0]]).T

# print a.controlInput(input)
