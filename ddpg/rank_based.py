...
class Experience(object):
    ...
    def __init__(self, conf):
        ...
        self.beta_grad = (1 - self.beta_zero) *1.0/ (self.total_steps - self.learn_start)
    def build_distributions(self):
        ...
        partition_num = 1
        # each part size
        partition_size = int(math.floor(self.size*1.0 / n_partitions))
        ...
        for n in range(partition_size, self.size + 1, partition_size):
            if self.learn_start <= n <= self.priority_size:
                ...
                distribution['pdf'] = list(map(lambda x: x *1.0/ pdf_sum, pdf))
